    ...
    try:
        ...
        for key, order in all_orders.items():
            ...
            if 0 < minutes_left <= 15:
                phone_suffix = user_phone[-4:]
                msg = f"{phone_suffix} 고객님, 주문하신 케이크 픽업 15분 전입니다! 🎂\n픽업시간: {pickup_dt.strftime('%H:%M')}"
...
def monitor_robot_status():
    global current_recording_order_id
    print("👀 [Monitor] 로봇 상태 감시 스레드 시작...")
    ...
    while True:
        try:
            # 1. 실시간 로봇 상태 읽기
            status_ref = db.reference('order_status')
            current_status = status_ref.get() # 'start', 'syrup', 'done' ...
            ...
            if current_status != last_status:
                ...
                if current_status == 'syrup':
                    # 현재 가장 최근 주문 번호 가져오기 (가장 큰 ID)
                    orders_data = db.reference('orders').order_by_key().limit_to_last(1).get()
                    if orders_data:
                        order_id = list(orders_data.keys())[0]
                        current_recording_order_id = order_id
                        recorder.start(order_id, camera_index=TIMELAPSE_CAM_INDEX)                    
                    else:
                        print("⚠️ [Camera] 주문 정보를 찾을 수 없어 녹화를 건너뜁니다.")

                # ⏹️ 작업 완료 -> 녹화 종료 및 업로드
                elif current_status == 'done':
                    if current_recording_order_id:
                        video_url = recorder.stop_and_upload(current_recording_order_id)
                        
                        if video_url:
                            # DB에 video_url 업데이트 (프론트엔드가 이걸 보고 QR 띄움)
                            db.reference(f'orders/{current_recording_order_id}').update({
                                'video_url': video_url
                            })
                            print(f"✨ [Success] 주문 #{current_recording_order_id} 영상 URL 저장 완료")
                        
                        current_recording_order_id = None # 초기화
...
async def startup_event():
    ...
    scheduler.add_job(check_pickup_alerts, 'interval', minutes=1)
    scheduler.start()

    # 👇 [여기 추가] 로봇 상태 감시 스레드 시작
    print("📹 [Startup] Starting Robot Status Monitor...")
    t_cam = threading.Thread(target=monitor_robot_status, daemon=True)
    t_cam.start()
